...
const uint8_t  num_analog_read = 1; 
...
//uint16_t CapReadings[num_cap_read];   // REPPLACE WITH ANALOG 
uint16_t SensorReadings[num_analog_read];

void setup() 
{
    steps_per_mm = steps_per_rev/pitch;   // = 100?
    ...
    if (Serial.available() > 1)   // since 2 byte (or more) writes 
    {
        ...
        switch (command)
        {
            ...
            case TEST:
                ...
                // Replace the contents of tet to oerform an anlog read instead              
...
void test(uint16_t distance_t10, uint16_t test_step_delay)
{
    ...
    uint32_t steps_t10 = (uint32_t) distance_t10 * steps_per_mm;    
    uint32_t num_steps = steps_t10/10;
    ...
    // Going Forward -- 
    for (uint32_t steps = 0; steps < num_steps; steps++)
    {           
        if ( ((cur_time - prev_time)  > sample_thresh) || (steps == 0))  
        {
            prev_time = cur_time; 

            // TODO -- get analog data 
            analogReadAll(1, 10); 
            data = SensorReadings[0]; 
    ...
    // Retractions 
    for (uint32_t steps = num_steps; steps > 0; steps--)
    {           
        if ( ((cur_time - prev_time)  > sample_thresh) || (steps == 0))  
        {
            prev_time = cur_time; 
...
void analogReadAll(uint8_t num_analog_read, uint8_t num_average)   // here do analog read instead 
{
     //uint8_t buffer_pos;
     for (uint8_t i = 0; i< num_analog_read; i ++)
     {
        //buffer_pos = 2 * i;
        uint16_t reading = 0;
        
        for (uint8_t j = 0; j < num_average; j++)
        {
          reading += analogRead(i); 
        }
     
        SensorReadings[i] = reading/num_average; 
     }

}

/*
void capRead(uint8_t num_cap_read)   // here do analog read instead 
{
